            ...
            <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
               @if (count($all_appointments) > 0)
                  @foreach ($all_appointments as $appointment)
                    <tr wire:key="appointment-{{ $appointment->id }}" class="bg-white hover:bg-gray-50 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                        @if (auth()->user() && auth()->user()->role == 0)
                      ...
                      @else
                        ...
                        <td class="size-px whitespace-nowrap align-top">
                          ...
                          @endphp
                          <div class="block p-6 space-y-2">
                            <span wire:key="status-{{ $appointment->id }}-{{ $status }}" wire:transition.opacity class="py-1 px-2 inline-flex items-center gap-x-1 text-xs font-medium rounded-full {{ $statusMeta['classes'] }}">
                              {{ $statusMeta['label'] }}
                            </span>
                            @if (auth()->user() && auth()->user()->role == \App\Models\User::ROLE_DOCTOR)
                              <select
                                class="mt-1 py-1.5 px-2 block w-full border-gray-200 rounded-lg text-xs focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400"
