  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div class="flex flex-col rounded-xl p-4 md:p-6 bg-white border border-gray-200 dark:bg-neutral-900 dark:border-neutral-700">
      <div class="flex items-center gap-x-4">
        <img src="{{ $item->image_url }}" alt="Doctor photo" class="inline-block size-[62px] rounded-full object-cover ring-2 ring-blue-100">
        <div class="grow">
          <h3 class="font-medium text-gray-800 dark:text-neutral-200">
            {{ str($item->doctorUser->name)->startsWith('Dr.') ? $item->doctorUser->name : 'Dr. ' . $item->doctorUser->name }}
          </h3>
      ...
      <div class="mt-3">
        ...
        <p class="text-sm font-semibold text-emerald-600">
          Next slot: {{$nextAvailability['day_label']}} • {{$nextAvailability['date']->format('d M, h:i A')}} – {{$nextAvailability['to']->format('h:i A')}}
        </p>
        @else
        <p class="text-sm text-gray-500">Availability coming soon.</p>
        @endif
        @if ($item->schedules->isNotEmpty())
        <div class="mt-2 flex flex-wrap gap-2">
          @foreach ($item->schedules->sortBy('available_day')->take(3) as $schedule)
          <span class="px-3 py-1 rounded-full text-xs bg-blue-50 text-blue-700">
            {{ \App\Models\Doctor::DAYS_OF_WEEK[$schedule->available_day] ?? 'Day' }}
            • {{ \Carbon\Carbon::createFromFormat('H:i:s', $schedule->from)->format('h:i A') }} -
            {{ \Carbon\Carbon::createFromFormat('H:i:s', $schedule->to)->format('h:i A') }}
          </span>
          @endforeach
          @if ($item->schedules->count() > 3)
          <span class="px-3 py-1 rounded-full text-xs bg-gray-100 text-gray-600">+{{ $item->schedules->count() - 3 }} more</span>
          @endif
        </div>
        @endif
      </div>
      <div class="flex justify-between mt-5">
        ...
        <!-- End Social Brands -->
        @php
        $prefillDate = $nextAvailability ? $nextAvailability['date']->format('Y-m-d') : null;
        $prefillSlot = $nextAvailability ? $nextAvailability['date']->format('H:i:s') : null;
        $bookingRoute = $nextAvailability
        ? "/booking/page/{$item->id}?date={$prefillDate}&slot={$prefillSlot}"
        : "/booking/page/{$item->id}";
        @endphp
        @if (auth()->user() != null)
        <a href="{{$bookingRoute}}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
          Book Appointment
        </a>
        @else
